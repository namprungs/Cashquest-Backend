      ...
      if (dto.simulation) {
        await tx.productSimulation.create({
          data: {
            ...
            model: dto.simulation.model ?? 'GBM',
            mu: dto.simulation.mu ?? 0,
            sigma: dto.simulation.sigma ?? 0,
            dt: dto.simulation.dt,
            faceValue: dto.simulation.faceValue,
            couponRate: dto.simulation.couponRate,
            initialYield: dto.simulation.initialYield,
            modifiedDuration: dto.simulation.modifiedDuration,
            kappa: dto.simulation.kappa,
            theta: dto.simulation.theta,
            sigmaYield: dto.simulation.sigmaYield,
            yieldFloor: dto.simulation.yieldFloor ?? 0.001,
          },
        });
      }
      ...
      for (const item of dto.items) {
        ...
        const simulation = await tx.productSimulation.upsert({
          ...
          update: {
            ...
            model: item.model ?? 'GBM',
            mu: item.mu ?? 0,
            sigma: item.sigma ?? 0,
            dt: item.dt,
            faceValue: item.faceValue,
            couponRate: item.couponRate,
            initialYield: item.initialYield,
            modifiedDuration: item.modifiedDuration,
            kappa: item.kappa,
            theta: item.theta,
            sigmaYield: item.sigmaYield,
            yieldFloor: item.yieldFloor ?? 0.001,
          },
          create: {
            termId,
            productId: item.productId,
